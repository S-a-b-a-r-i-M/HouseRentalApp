...
fun getDummyPropertyEntity(index: Int, userId: Long) =
    PropertyEntity(
        ...
        address = PropertyAddressEntity(
            ...
            locality = listOf("tambaram", "sri rangam", "vannemalai", "ecr", "kodambakkam", "air port", "maattu thavani").random(),
...
fun insertUsers(db: SQLiteDatabase): List<Long> {
    ...
    return listOf(values1, values2, values3).map {
        db.insertOrThrow(UserTable.TABLE_NAME, null, it)
    }
}

fun insertInitialData(db: SQLiteDatabase) {
    val userIds = insertUsers(db)
    userIds.forEach { userId ->
        for (i in 0..500) {
            val entity = getDummyPropertyEntity(i, userId)
